...
def upload_file(file: UploadFile, user_mail: EmailStr, start_datetime: datetime, end_datetime: datetime):
	user_id = 0
	#поиск почты в бд, если всё в порядке, получаем id пользователя, запоминаем в переменную user_id
	storage = FileStorage()
