...
def plot_maps(prod_files, prods, epc, clims, times, lat_limits, lon_limits, nrows, ncols, savefig='', scale=1,
              test_mod=False):
    C_LIMITS = clims
    if scale != 1:
        for k in C_LIMITS:
...
def plot_distance_time(x, y, c, ptype, epcs, sort = True, line=dict(), clims=C_LIMITS, dmax=1750, data=[], savefig=''):
